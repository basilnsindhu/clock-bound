...
/// The default clock frequency used for calculating min/max duration
pub const DEFAULT_CLOCK_FREQUENCY: u64 = 11; //11ppm
...
impl ClockBoundClient {
    ...
    pub fn timing<A, F>(&self, f: F) -> Result<(TimingResult, A), (ClockBoundCError, Result<A,F>)>
        ...
        let response_earliest_start = NetworkEndian::read_u64(&response[4..12]);
        let response_earliest_finish = NetworkEndian::read_u64(&response[12..20]);
        ...
        let response_latest_start = NetworkEndian::read_u64(&response[4..12]);
        let response_latest_finish = NetworkEndian::read_u64(&response[12..20]);

        // Calculate elapsed time between early and latest timestamps
        let earliest_time_elapsed_midpoint = (response_earliest_finish - response_earliest_start)/2;
        let latest_time_elapsed_midpoint = (response_latest_finish - response_latest_start)/2;

        let earliest_start = UNIX_EPOCH + Duration::from_nanos(response_earliest_start);
        let latest_start = UNIX_EPOCH + Duration::from_nanos(response_latest_start);
        let latest_finish = UNIX_EPOCH + Duration::from_nanos(response_latest_finish);

        // Add half of time elapsed to the start times to get the mid points
        let duration_start = earliest_start.checked_add(Duration::from_nanos(earliest_time_elapsed_midpoint)).unwrap();
        let duration_finish = latest_start.checked_add(Duration::from_nanos(latest_time_elapsed_midpoint)).unwrap();

        // Calculates duration between the two midpoints
        let duration = duration_finish.duration_since(duration_start).unwrap();
        let ppm = Duration::from_micros(DEFAULT_CLOCK_FREQUENCY);

        Ok((TimingResult{
            earliest_start,
            latest_finish,
            min_execution_time: duration.saturating_sub(ppm),
            max_execution_time: duration.saturating_add(ppm)
        }, callback))
    }
}
